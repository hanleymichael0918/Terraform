...
resource "azurerm_virtual_network_peering" "test2" {
  ...
  resource_group_name       = "sqlpocwest"
  virtual_network_name      = "${azurerm_virtual_network.VirtualNetworkWest.name}"
...
resource "azurerm_network_interface" "DCNic1" {
  ...
  depends_on           =  ["azurerm_virtual_machine.DC"]
    ...
    {
        "fileUris": [
           "https://sapowershellcontainer.blob.core.windows.net/scripts/Initialise-VM.ps1?st=2019-01-02T16%3A10%3A43Z&se=2019-01-04T19%3A10%3A00Z&sp=rl&sv=2018-03-28&sr=b&sig=JFJVAaieDFEB8i4pNMM31vjVisXk5DIqG1R9O2ACxvo%3D"
        ],
        "commandToExecute": "powershell -ExecutionPolicy Unrestricted -File \"Initialise-VM.ps1\""
    }
    SETTINGS
}